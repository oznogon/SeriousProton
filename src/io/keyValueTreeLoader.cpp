...
KeyValueTreeLoader::KeyValueTreeLoader(const string& resource_name)
{
    ...
    if (!stream)
    {
        LOG(Error, "[sp-kvtl] Failed to open " + resource_name + " for tree loading.");
        return;
    }
    result = std::make_shared<KeyValueTree>();

    LOG(Info, "[sp-kvtl] Loading theme tree: ", resource_name);
    ...
    while (stream->tell() < stream->getSize())
    {
        ...
        {
            LOG(Error, "[sp-kvtl] Failed to parse key/value tree. Node close while no node open. Resource: ", resource_name);
            result = nullptr;
            return;
        }
        ...
        {
            LOG(Error, "[sp-kvtl] Failed to parse line: ", line);
            result = nullptr;
            return;
        }
    }
