...
public:
    ...
    {
        if (!bool(*this) || !hasComponent<T>()) return nullptr;
    ...
    // You should never need this, but the multiplayer code does.
    uint32_t getIndex() const { return index; }
    // You should never need this, but the multiplayer code does.
    uint32_t getVersion() const { return version; }

    static Entity forced(uint32_t index, uint32_t version)
    {
        auto e = Entity();
        e.index = index;
        e.version = version;
...
private:
    ...
    // These need to be friends for network replication.
