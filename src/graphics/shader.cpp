...
: attribute_mapping{attribute_mapping}, name(name)
{
    ...
    for (auto line : code.split("\n"))
    {
        ...
        else if (mode == 0) vertex_code += line + "\n";
        else if (mode == 1) fragment_code += line + "\n";
...
static unsigned int compileShader(const string& name, const char* header, const char* code, int type)
{
    ...
    if (!success)
    {
        ...
        LOG(Error, "[sp-shader] Compile error in shader: ", name, ":", log);
...
bool Shader::compileShader()
{
    ...
    if (!vertex_shader_handle) return false;
    ...
    if (!success)
    {
        ...
        LOG(Error, "[sp-shader] Link error in shader: ", name, log);
...
int Shader::getUniformLocation(const string& name)
{
    ...
    if (it != uniform_mapping.end()) return it->second;

    int location = glGetUniformLocation(program, name.c_str());
    if (location == -1)
        LOG(Debug, "[sp-shader] Failed to find uniform: ", name, " in ", this->name);
...
int Shader::getAttributeLocation(const string& name)
{
    ...
    if (it != attribute_mapping.end()) return it->second;

    int location = glGetAttribLocation(program, name.c_str());
    if (location == -1)
        LOG(Debug, "[sp-shader] Failed to find attribute: ", name, " in ", this->name);
