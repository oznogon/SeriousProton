...
FreetypeFont::FreetypeFont(const string& name, P<ResourceStream> stream)
{
    ...
    LOG(Info, "[sp-freetype] Loading font: ", name);

    FT_Library library;
    if (FT_Init_FreeType(&library) != 0)
    {
        LOG(Error, "[sp-freetype] Failed to initialize freetype library");
        return;
    }
    ...
    {
        LOG(Error, "[sp-freetype] Failed to create font from stream.");
    ...
    {
        LOG(Error, "[sp-freetype] Failed to select Unicode for font.");
        FT_Done_Face(face);
        delete stream_rec;
        FT_Done_FreeType(library);
        return;
...
bool FreetypeFont::getGlyphInfo(int char_code, int pixel_size, Font::GlyphInfo& info)
{
    ...
    if (known_glyphs.find(char_code) == known_glyphs.end())
    {
        ...
        if (glyph_index == 0 || FT_Load_Glyph(face, glyph_index, FT_LOAD_DEFAULT) != 0)
            LOG(Warning, "[sp-freetype] Failed to find glyph in font: ", "0x" + string::hex(char_code));
        else
        {
            FT_Glyph glyph;
            if (FT_Get_Glyph(face->glyph, &glyph) == 0)
            {
                info.advance = static_cast<float>(face->glyph->metrics.horiAdvance) / static_cast<float>(1 << 6);
                info.bounds.position.x = static_cast<float>(face->glyph->metrics.horiBearingX) / static_cast<float>(1 << 6);
                info.bounds.position.y = static_cast<float>(face->glyph->metrics.horiBearingY) / static_cast<float>(1 << 6);
                info.bounds.size.x = static_cast<float>(face->glyph->metrics.width) / static_cast<float>(1 << 6);
                info.bounds.size.y = static_cast<float>(face->glyph->metrics.height) / static_cast<float>(1 << 6);
...
Image FreetypeFont::drawGlyph(int char_code, int pixel_size)
{
    ...
    if (glyph_index != 0 && FT_Load_Glyph(face, glyph_index, FT_LOAD_DEFAULT) == 0)
    {
        ...
        if (FT_Get_Glyph(face->glyph, &glyph) == 0)
        {
            ...
            if (bitmap.pixel_mode == FT_PIXEL_MODE_MONO) SDL_assert(false); //TODO
            else
            {
...
float FreetypeFont::getLineSpacing(int pixel_size)
{
    ...
    return static_cast<float>(static_cast<FT_Face>(ft_face)->size->metrics.height) / static_cast<float>(1 << 6);
}
