...
public:
    ...
    // Call this to check if the timer has expired. This returns true only once
    ...
    // Return the timer progress in the range 0.0 to 1.0, where 0.0 is at the
    // start.
...
// The normal timer runs at the game speed. If the game is paused or slowed
// down, this timer slows down as well.
using Timer = TimerBase<EngineTime>;
using Stopwatch = StopwatchBase<EngineTime>;

// The system timer always runs, even if the game is paused or at a different
// game speed.
